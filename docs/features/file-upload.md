...
## Purpose
Ingest seed images from the browser into Backblaze B2 with real-time progress tracking — the entry point for the augmentation pipeline. Uploaded files land under `uploads/`; point a recipe's seed prefix at them (or at any prefix you stage seeds under).
