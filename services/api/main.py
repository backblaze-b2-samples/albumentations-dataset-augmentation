...
app = FastAPI(
    title="Albumentations Augmentation Pipeline API",
    description=(
        "Reproducible image dataset augmentation API backed by Backblaze B2. "
        "Define versioned Albumentations recipes and run them to expand seed "
        "images into N augmented training variants stored in B2."
    ),
    version="0.1.0",
    lifespan=lifespan,
)
...
app.include_router(transforms.router, tags=["transforms"])
app.include_router(recipes.router, tags=["recipes"])
app.include_router(runs.router, tags=["runs"])
app.include_router(augment_stats.router, tags=["augment-stats"])
