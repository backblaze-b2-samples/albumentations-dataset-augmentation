...
This app is an **image dataset augmentation pipeline**: define a versioned
Albumentations *recipe* over seed images in B2, run it to write N augmented
variants per image back to B2, with a reproducibility manifest per run. The
augmentation engine (`services/api/app/service/augment.py`) is pure compute —
all B2 I/O stays in `repo/`.

## 1. Repository Map

```
apps/web/          Next.js 16 frontend (App Router, Tailwind v4, shadcn/ui)
  src/app/recipes/        Recipes list + [id] detail (create/read/edit/delete/run)
  src/app/runs/           /runs/[recipeId]/[runId] run detail (gallery, manifest, re-run)
  src/app/gallery/        Scoped augmented/ output explorer
  src/components/recipes/  recipe-form, recipe-list, recipe-detail
services/api/      FastAPI backend (layered: types/config/repo/service/runtime)
  app/service/augment.py   Albumentations engine (pure compute — NO boto3)
  app/service/recipes.py   Recipe manifest CRUD + validation
  app/service/runs.py      Run orchestration + run manifests
  app/service/transforms.py  Curated transform catalog
  app/repo/b2_client.py    All boto3/S3 I/O (get_object_bytes/put_bytes/list_object_keys added)
...
- **Dashboard.** `/` route and `apps/web/src/components/dashboard/` have been rewritten for **augmentation metrics**: seed-image count, recipe count, total variants, dataset multiplication factor, bytes written to B2, a recent-runs table, and a "variants produced per run" chart. Aggregations flow `runtime -> service -> repo` (`GET /augment/stats` -> `service/augment_stats.py`) and surface via TanStack Query hooks in `apps/web/src/lib/queries.ts` — no bare `useEffect + fetch`.
- The starter dashboard component filenames are reused (`stats-cards.tsx`, `upload-chart.tsx`, `recent-uploads-table.tsx`) with augmentation content.
