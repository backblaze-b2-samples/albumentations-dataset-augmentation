...
export async function getTransforms() {
  return apiFetch<TransformSpec[]>("/transforms");
}
...
export async function updateRecipe(id: string, payload: RecipeCreate) {
  return apiFetch<Recipe>(`/recipes/${id}`, {
    method: "PUT",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify(payload),
...
export async function deleteRecipe(id: string) {
  return apiFetch<{ deleted: boolean; id: string }>(`/recipes/${id}`, {
    method: "DELETE",
  });
}
...
export async function runRecipe(
  id: string,
  body: { seed_prefix?: string | null; random_seed?: number | null } = {}
) {
  return apiFetch<RunResult>(`/recipes/${id}/run`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
...
export async function getRuns(recipeId: string) {
  return apiFetch<RunSummary[]>(`/recipes/${recipeId}/runs`);
}

export async function getRunDetail(recipeId: string, runId: string) {
  return apiFetch<RunDetail>(`/recipes/${recipeId}/runs/${runId}`);
}

export async function getAugmentStats() {
  return apiFetch<AugmentStats>("/augment/stats");
}

export async function getGallery() {
  return apiFetch<GalleryRun[]>("/gallery");
}

export async function getObjectPreview(key: string) {
